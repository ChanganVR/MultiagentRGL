...
class ValueActionPredictor(nn.Module):
    def __init__(self, config, graph_model_val, graph_model_act=None, shared_gcn=True):
        ...
        self.value_network = mlp(config.gcn.X_dim, config.rgl_actor_critic.value_network_dims)
        self.action_network = mlp(config.gcn.X_dim, config.rgl_actor_critic.action_network_dims) 
